    ...
    \item For a trial $i$ of the meta-analysis with $k$ trials, $i = 1, \dots, k$:
        \begin{enumerate}
            \item Simulate the true effect size using the Gaussian model:
                ...
                %% We use $\alpha=4$ where $\delta \approx \Sexpr{round(4/sqrt(1+4^2), 2)}$. 
...
$\theta=\Sexpr{theta}$ and variance $\tau^2=\Sexpr{round(sqrt(tau2), 3)}^2$. The median of each distribution is indicated with a dashed lined. \label{fig:fig1}}
\end{figure}
...
based on the DerSimonian-Laird \citep{ders:lair:86} estimate of the between-study variance $\tau^2$.
...
We also calculate the following measures related to skewness:
\begin{enumerate}
    \item Agreement of the sign of the (weighted) skewness coefficient of the observed study effects and the sign of the skewness of the confidence interval.   
\end{enumerate}
